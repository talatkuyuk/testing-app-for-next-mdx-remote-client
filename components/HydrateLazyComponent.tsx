import { hydrateLazy, type SerializeResult } from "next-mdx-remote-client/csr";
 ...
 */
const HydrateLazyComponent = ({ mdxSource, data }: Props) => {
  ...
  const { content, mod, error } = hydrateLazy({ ...mdxSource, components });
  ...
  return (
    <>
      ...
      <span className="left-corner-absolute-note">
        <strong suppressHydrationWarning>{proofForExports}</strong>
      </span>
    </>
  );
};

export default HydrateLazyComponent;
